...
public class ClientEx {
	...
	private static final int SERVER_PORT = 12145; // 서버가 열어놓은 포트번호 (서버와 클라이언트의 포트번호는 같아야함)

	public static void main(String[] args) {
		String serverHostname = "127.0.0.1"; // Localhost = 127.0.0.1 = 루프백
		isRunning = true;
		// 서버와 연결이 된다면 연결하고, 서버에 연결되어있는 방문자는 한 번 연결하면 끝이긴 하지만
		// 서버에서 강제로 종료하고 다시 접속하려면 다시 여기를 타고 null 값을 보여줌
		Thread serverThread = new Thread(() -> {
			// 서버에서 클라이언트한테 메세지를 보내는 값으로 처음에는 아무 대화도 없기 때문에 null을 줌
			ServerToClientThread serverToClientThread = null;

			// 클라이언트가 서버한테 메세지를 보내는 값으로 처음에는 아무 대화도 없기 때문에 null을 줌
			...
			while (isRunning) {
				...
				try {
					// 서버랑 클라이언트의 연결이 끊어졌다면 1초마다 재접속 시도
					Thread.sleep(1000);
