...
getInputStream() : 소켓에서 데이터(글자, 파일 등)를 읽어오기 위한 입력 스트림
getOutputStream() : 소켓에서 가져온 데이터(글자, 파일 등)를 쓰기위한 출력 스트림

close() : 소켓을 닫아 연결을 종료

BufferedReader
readLine() : 텍스트를 한줄씩 읽어올 때 사용, 주로 대화나 파일 주고 받을 때 사용

ServerSocket
메서드 중에서 우리집에 어떻게 들어올지 포트번호를 지정하는 필수 생성자로 많이 사용

PrintWriter : 글로 작성한 내용을 출력
